...
static void find_create(void);
static int find_clicked(GtkButton *button, gpointer data);
static gint find_key_press_event(GtkWidget *widget, GdkEventKey *event,gpointer user_data);
void find_destroy(GtkWidget *widget, gpointer data);


static void replace_create(void);
static void replace_clicked(GtkButton *button, gpointer data);
static void replace_prompt_reply(gint reply, gpointer data);
static void replace_clicked(GtkButton *button, gpointer data);
static void replace_all_clicked(GtkButton *button, gpointer data);

static FindDialog *find_dialog=NULL;
static ReplaceDialog *replace_dialog=NULL;
static GtkWidget *replace_prompt_dialog=NULL;


void find_destroy(GtkWidget *widget, gpointer data){
	gtk_widget_destroy(find_dialog->window);
	uber_free(find_dialog);
}//find_destroy
...
static int find_clicked(GtkButton *button, gpointer data){
	...
	length_of_document = gtk_scintilla_get_length(GTK_SCINTILLA(editor->scintilla));
	...
	if(whole_document) {
		current_pos = 0;
		gtk_scintilla_goto_pos(GTK_SCINTILLA(editor->scintilla), current_pos);
	...
	result = gtk_scintilla_find_text(GTK_SCINTILLA(editor->scintilla),
	...
	gtk_scintilla_goto_pos(GTK_SCINTILLA(editor->scintilla), start_found);	
	gtk_scintilla_set_selection_start(GTK_SCINTILLA(editor->scintilla), start_found);
	gtk_scintilla_set_selection_end(GTK_SCINTILLA(editor->scintilla), end_found);
	return TRUE;
}//find_clicked
...
void find_show(void){
	if(!(find_dialog && find_dialog->window))
		find_create();
	
	gtk_window_present(GTK_WINDOW(find_dialog->window));
}/*find_show();*/

static void find_create(void){
	find_dialog=g_new0(FindDialog, 1);
	Editor *editor=main_window_get_current_editor();
	...
	find_dialog->window = gtk_window_new(GTK_WINDOW_TOPLEVEL);
	gtk_window_set_position(GTK_WINDOW(find_dialog->window),GTK_WIN_POS_CENTER);
	gtk_window_set_transient_for(GTK_WINDOW(find_dialog->window), GTK_WINDOW(main_window_get_window()));
	gtk_container_set_border_width(GTK_CONTAINER(find_dialog->window), 6);
	gtk_window_set_title(GTK_WINDOW(find_dialog->window), _("Find"));
	gtk_window_set_resizable(GTK_WINDOW(find_dialog->window), FALSE);

	find_dialog->vbox1 = gtk_vbox_new(FALSE, 0);
	gtk_widget_show(find_dialog->vbox1);
	gtk_container_add(GTK_CONTAINER(find_dialog->window), find_dialog->vbox1);
	...
	gtk_signal_connect(GTK_OBJECT(find_dialog->window), "key_press_event", GTK_SIGNAL_FUNC(find_key_press_event), NULL);

	gtk_signal_connect_object(GTK_OBJECT(find_dialog->button4), "clicked", GTK_SIGNAL_FUNC(find_destroy), NULL);
							   
	gtk_signal_connect(GTK_OBJECT(find_dialog->window), "destroy", GTK_SIGNAL_FUNC(find_destroy), NULL);
}//find_create

// -----------------------------------------------------------------------------

static void replace_destroy(GtkWidget *widget, gpointer data){
	gtk_widget_destroy(replace_dialog->window);
	uber_free(replace_dialog);
}//replace_destroy
...
static void replace_prompt_reply(gint reply, gpointer data){
	...
	if(!reply) { // YES
		...
		selection_start = gtk_scintilla_get_selection_start(GTK_SCINTILLA(editor->scintilla));
		gtk_scintilla_replace_sel(GTK_SCINTILLA(editor->scintilla), replace);
		gtk_scintilla_set_selection_start(GTK_SCINTILLA(editor->scintilla), selection_start);
		gtk_scintilla_set_selection_end(GTK_SCINTILLA(editor->scintilla), selection_start + strlen(replace));
	}
...
static void replace_clicked(GtkButton *button, gpointer data){
	...
	gtk_scintilla_goto_pos(GTK_SCINTILLA(editor->scintilla), start_found);	
	gtk_scintilla_set_selection_start(GTK_SCINTILLA(editor->scintilla), start_found);
	gtk_scintilla_set_selection_end(GTK_SCINTILLA(editor->scintilla), start_found + strlen(replace));
}//replace_clicked
...
static void replace_all_clicked(GtkButton *button, gpointer data){
	...
	gtk_widget_hide(replace_dialog->window);
	...
	start_pos = gtk_scintilla_get_current_pos(GTK_SCINTILLA(editor->scintilla));
	...
	gtk_scintilla_goto_pos(GTK_SCINTILLA(editor->scintilla), start_pos);
	replace_destroy(NULL, NULL);
}//replace_all_clicked


static gint replace_key_press_event(GtkWidget *widget, GdkEventKey *event,gpointer user_data){
	if(event->keyval == GDK_Return) {
		replace_clicked(NULL,NULL);
		return TRUE;
	}
...
void replace_show(void){
	if(!( replace_dialog && replace_dialog->window ))
		replace_create();
	
	gtk_window_present(GTK_WINDOW(replace_dialog->window));
}/*replace_show();*/

static void replace_create(void){
	replace_dialog=g_new0(ReplaceDialog, 1);
	Editor *editor=main_window_get_current_editor();
	...
	replace_dialog->window = gtk_window_new(GTK_WINDOW_TOPLEVEL);
	gtk_window_set_position(GTK_WINDOW(replace_dialog->window),GTK_WIN_POS_CENTER);
	gtk_container_set_border_width(GTK_CONTAINER(replace_dialog->window), 6);
	gtk_window_set_title(GTK_WINDOW(replace_dialog->window), _("Find and Replace"));

	replace_dialog->vbox10 = gtk_vbox_new(FALSE, 0);
	gtk_widget_show(replace_dialog->vbox10);
	gtk_container_add(GTK_CONTAINER(replace_dialog->window), replace_dialog->vbox10);
	...
	gtk_signal_connect(GTK_OBJECT(replace_dialog->window), "key_press_event", GTK_SIGNAL_FUNC(replace_key_press_event), NULL);
	...
	gtk_signal_connect(GTK_OBJECT(replace_dialog->window), "destroy", GTK_SIGNAL_FUNC(replace_destroy), NULL);
}//replace_create
